...
Downloads VITS Japanese TTS model files from HuggingFace Hub into a local cache
directory. Performs a minimum-size sanity check on cached files and, optionally,
quantizes the downloaded model to int8 via ONNX Runtime dynamic quantization for
fast CPU inference.

Called from main.py only when TTS_ENGINE=vits_ja — the Kokoro path is unaffected.
"""
