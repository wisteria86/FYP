...
const ENGINE_LABELS = {
  chattts: { label: 'ChatTTS', flag: '🗣️', lang: 'English / Chinese' },
...
export default function App() {
  ...
  // Record in the browser and transcribe through the local Whisper API.
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          setMessages(prev => [...prev, { id: Date.now(), role: 'bot', text: error.message, isError: true }])
        }
      }
      recorder.start()
      setIsListening(true)
      ...
      setIsListening(false)
      setMessages(prev => [...prev, { id: Date.now(), role: 'bot', text: 'Microphone permission was denied.', isError: true }])
    }
  }
  ...
  const enqueueAudio = useCallback((audioB64) => {
    if (!audioB64) return
    ...
    const playNext = () => {
      ...
      if (!next) { audioRef.current = null; return }
      const raw = atob(next)
      const buf = new Uint8Array(raw.length)
      for (let i = 0; i < raw.length; i++) buf[i] = raw.charCodeAt(i)
      const url = URL.createObjectURL(new Blob([buf], { type: 'audio/wav' }))
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (event.type === 'sentence') {
            ...
            setMessages(prev => prev.map(msg => msg.id === botId ? { ...msg, text: assembled } : msg))
